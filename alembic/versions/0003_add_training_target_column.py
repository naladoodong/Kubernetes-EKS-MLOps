...
def upgrade() -> None:
    ...
    existing = connection.execute(sa.text("SELECT EXISTS (SELECT 1 FROM training_jobs)")).scalar()
    if existing:
        raise RuntimeError("training_jobs rows require an explicit target_column_id backfill before this migration")
